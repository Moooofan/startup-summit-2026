    ...
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      ...
      <div className="absolute inset-0 hidden md:block">
        ...
        >
          ...
          {/* 整組右移 150（業主 2026/9：「4 跟裂縫重疊過多」）。

              成因不是位置差一點，是**兩條線幾乎共線**：斜筆的斜邊由 (620,900) 走到
              (1210,150)，而當時 HomeBackdrop 的斜線分割邊由 (650,900) 走到 (1260,-60) ——
              實測 y=700 處兩者 x 完全相同，整個高度範圍內最遠也只差 83 單位。
              兩條粗細相近的斜線疊在一起，讀起來就是一團糊掉的東西而不是兩個元素。

              註：HomeBackdrop 2026/9 已改成滿版不透明照片、那條斜線不存在了，首頁第一屏
              也看不到本層。但右移**不要撤回** —— 這一層是全站背景，其他頁（/speakers、
              /agenda…）的構圖平衡已經依現在的位置調過，撤回等於改動所有頁面的背景。

              移動整組而不是只移斜筆：斜筆的頂點與右緣色塊、弧帶是同一組相對關係，
              單獨推斜筆會讓它撞進色塊裡。右移之後右緣的色塊與弧帶會多出血一些，
              那兩者本來就是貼著邊界設計的，不影響構圖。
              這一層是全站背景，所以其他頁也會跟著右移 —— 幅度只有畫面寬的約 9%。 */}
      ...
      {/* 2b／線構圖・直式（手機）。
          ...
          opacity 走過 0.5 -> 0.72 -> 0.9 -> 1（使用者逐次目視定案）。0.5 是為了壓住
          「被 xMax 裁切後放得過大的弧帶」，那個成因已隨直式構圖消失；0.72、0.9 實測都還太弱。
          現在整層不再折減，強度全部交給各圖形自己的 stopOpacity / strokeOpacity ——
          少一層乘數，日後微調某一個元素時不會被一個全域係數稀釋掉。

          滿版是有算過的，不是放棄控制：斜筆最亮處合成後白字仍有 8.2:1、ink-2 有 5.5:1，
          離 AA 還很遠。**可讀性從來不是這裡的限制條件**，別為了保險把它調回去；
          真的要收，優先收 kv-wedge-m 的 stopOpacity，那才是面積最大的那一塊。 */}
      <div className="absolute inset-0 opacity-100 md:hidden">
        <svg
          className="h-full w-full"
